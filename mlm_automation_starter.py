...
def upsert_lead_from_systeme(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    contact = data.get("contact") or data.get("customer") or {}
    fields = contact.get("fields") or {}

    name_parts = [fields.get("first_name"), fields.get("surname")]
...
async def systeme_webhook(
    ...
    # Depending on your setup, you may validate that secret through headers,
    # query params, or middleware. This starter checks a header if provided,
    # and otherwise just accepts the event so you can test end-to-end.
